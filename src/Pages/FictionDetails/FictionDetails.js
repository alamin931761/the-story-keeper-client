...
const FictionDetails = () => {
    ...
    const { image, name, name2, author, price, description, publisher, publication_date, weight, pages_quantity, dimensions, isbn, binding } = fictionDetails;
    const { id } = useParams();
    useEffect(() => {
        fetch(`http://localhost:5000/book/${id}`)
            .then(res => res.json())
            .then(data => setFictionDetails(data))
    }, []);
