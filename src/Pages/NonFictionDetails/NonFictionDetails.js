...
const NonFictionDetails = () => {
    const [nonFictionDetails, setNonFictionDetails] = useState([]);
    const { image, name, name2, author, price, description, publisher, publication_date, weight, pages_quantity, dimensions, isbn, binding } = nonFictionDetails;
    const { id } = useParams();
    useEffect(() => {
        fetch(`http://localhost:5000/book/${id}`)
            .then(res => res.json())
            .then(data => setNonFictionDetails(data))
    }, []);
    return (
