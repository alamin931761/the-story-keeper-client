...
const NewArrivals = () => {
    ...
    const { size, page, setPage, setSliderValue, sliderValue, setSize } = useContext(NEW_ARRIVAL_CONTEXT);
    ...
    return (
        <div className='common-style' data-aos="fade-down" data-aos-duration="1000">
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                {
                    newArrivals.map(data => <Card key={data._id} data={data}></Card>)
                }
            </div>
            ...
            <div className='text-center my-6'>
                {
                    [...Array(pages).keys()].map(number => <button key={number} onClick={() => setPage(number)} className={`${page === number ? 'btn btn-warning' : 'btn'} my-2 mx-2`}>{number + 1}</button>)
                }
            </div>
        </div>
    );
